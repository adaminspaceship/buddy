...
interface PluginConfig {
  ...
  apiKey?: string;
  model?: string;
  transcriptionProvider?: "elevenlabs" | "openai";
  routePrefix?: string;
  audioField?: string;
}
      ...
      handler: async (req: IncomingMessage, res: ServerResponse) => {
        ...
        setImmediate(async () => {
          try {
            ...
            const allowFrom: string[] = runtimeCfg?.channels?.whatsapp?.allowFrom ?? [];
...
async function transcribe(filePath: string, mime: string, cfg: PluginConfig): Promise<string> {
